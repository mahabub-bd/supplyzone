...
export default function CustomersReportView() {
  ...
  const [fetchReport, { isLoading, isError }] = useLazyGetCustomersReportQuery();
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      ...
      if (result?.data) {
        ...
        setIsModalOpen(true);
        toast.success("Customers report generated successfully");
      }
    } catch (error: any) {
      toast.error(error?.data?.message || "Failed to generate customers report");
    }
  };
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle="Generate and view customers reports"
        className="mb-6"
      />
      ...
      {/* Loading State */}
      {isLoading && <Loading message="Generating customers report..." />}
      ...
      {isError && (
        <div className="rounded-xl border border-red-200 bg-red-50 p-6 text-red-700 dark:border-red-800 dark:bg-red-900/20 dark:text-red-400">
          Failed to load customers report. Please try again.
        </div>
      )}
      ...
      {isModalOpen && reportData && (
        <Modal
          ...
          title="Customers Report Details"
          className="max-w-4xl"
        >
