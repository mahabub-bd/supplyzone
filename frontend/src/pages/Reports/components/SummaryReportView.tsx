...
export default function SummaryReportView() {
  ...
  const [fetchReport, { isLoading, isError }] = useLazyGetSummaryReportQuery();
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      ...
      toast.error(error?.data?.message || "Failed to generate summary report");
    }
  };
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle="Generate and view summary reports"
        className="mb-6"
      />
      ...
      {isError && (
        <div className="rounded-xl border border-red-200 bg-red-50 p-6 text-red-700 dark:border-red-800 dark:bg-red-900/20 dark:text-red-400">
          Failed to load summary report. Please try again.
        </div>
      )}
